...
def key_schedule(key):
    ...
    L, P = PC1_key[:28], PC1_key[28:]

    subkeys = []
    for shift in shift_table:
        L, P = shift_left(L, shift), shift_left(P, shift)
        PC2_key = permute(L + P, PC2)
        subkeys.append(PC2_key)
...
def Feistel(message, subkeys, F):
    ...
    for r in range(16):
        L, P = message[:32], message[32:]
        L = xor(L, F(P, subkeys[r]))
        if (r < 15):
            L, P = P, L
        message = L + P
...
def F(right, subkey):
    permuted = permute(right, E)
    xored = xor(permuted, subkey)
    groups = [xored[i: i+6] for i in range(0, 48, 6)]

    def sbox_group(idx, group):
        row = bin2dec(group[0] + group[5])
        column = bin2dec(group[1:5])
        return dec2bin(SBox[idx][row][column], 4)

    sboxed = [sbox_group(*group) for group in enumerate(groups)]
    sboxed = ''.join(sboxed)
...
def split_message(message, size=64):
    def chunks(lst, n):
        for i in range(0, len(lst), n):
            yield lst[i:i + n]

    return list(chunks(message, size))


def DES(message, subkeys):
    IP_permute = permute(message, IP)
    feisteled = Feistel(IP_permute, subkeys, F)
    return permute(feisteled, FP)
